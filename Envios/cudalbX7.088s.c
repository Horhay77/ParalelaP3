...
#define currentGPU 0
#define BLOCK_DIM_FILAS 64
#define BLOCK_DIM_COLUMNAS 16
...
int* maxDistance(const int* mapa, const unsigned int numfil, const unsigned int numcols, const dim3 grid, const dim3 block){
    ...
    /*if(nuevas==49){
        int *filreduc = (int*) malloc((size_t) BLOCK_DIM_FILAS * grid.x * grid.y * sizeof(int) ); 
        cudaMemcpy(filreduc, col_Result,  BLOCK_DIM_FILAS * grid.x * grid.y * sizeof(int), cudaMemcpyDeviceToHost);
        for(int i = 0; i < BLOCK_DIM_FILAS * grid.x * grid.y;i++){
            if(!(i%32))
                printf("\n");
            printf("%d ",filreduc[i]);
        }
    ...
    /*if(nuevas==49){
        int *filreduc = (int*) malloc((size_t) BLOCK_DIM_FILAS * grid.x * grid.y * sizeof(int) );
        cudaMemcpy(filreduc, val_Result,  BLOCK_DIM_FILAS * grid.x * grid.y * sizeof(int), cudaMemcpyDeviceToHost);
        for(int i = 0; i < grid.x * grid.y;i++){
            printf("%d ",filreduc[32*i]);
        }
        printf("\n");
        cudaMemcpy(filreduc, row_Result,  grid.x * grid.y * sizeof(int), cudaMemcpyDeviceToHost);
        for(int i = 0; i < grid.x * grid.y;i++){
            printf("%d ",filreduc[i]);
        }
        printf("\n");
    ...
    if(numThreadsPerBlock > 1024){
        while( (numThreadsPerBlock -= 1024) > 0 )
            blocks++;
