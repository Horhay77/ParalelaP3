...
__global__ void reduce_rows_kernel( int*, int*, int*, const int);

__global__ void reduce_blocks_kernel(int* vals, int* rows, int* cols, const unsigned int lastBlock, const unsigned int);
...
__global__ void reduce_blocks_kernel(int* vals, int* rows, int* cols, const unsigned int lastBlock, const int reductionLevel){
    extern __shared__ unsigned int sdata[];
    unsigned int tid = threadIdx.x;
...
int* maxDistance(const int* mapa, const unsigned int numfil, const unsigned int numcols, const dim3 grid, const dim3 block){
    ...
    do{
        ...
        if(numThreadsPerBlock > 1024){
            ...
            reduce_blocks_kernel<<<blocks, MAX_THREADS, sharedMemorySize>>>(val_Result, row_Result, col_Result, blocks * 1024 + numThreadsPerBlock % 1024, inception);
            ...
            blocks = 1;
            sharedMemorySize = 3 * numThreadsPerBlock * sizeof(int);
            reduce_blocks_kernel<<<blocks, numThreadsPerBlock, sharedMemorySize>>>(val_Result, row_Result, col_Result, numThreadsPerBlock, inception);
